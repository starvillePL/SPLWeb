...
export default function Table() {
    let fortuna = [1, 'FTN', 2, 2, 0, 0, 67, 10]
    let atletico = [2, 'ATL', 2, 2, 0, 0, 61, 12]
    let AC = [3, 'ACS', 2, 1, 0, 1, 18, 30]
    let east = [4, 'EFC', 2, 1, 0, 1, 11, 17]
    let comets = [5, 'CMT', 2, 1, 0, 1, 18, 33]
    let central = [6, 'CNT', 2, 1, 0, 1, 16, 50]
    let OL = [7, 'OLS', 2, 0, 0, 2, 12, 22]
    let arlington = [8, 'ARS', 2, 0, 0, 2, 8, 47]
                ...
                {teams.map(function(object, i){
                    return (
                        <div key={i} className="tableContainer">
                            ...
                            <div className="GD"><p>{sortedTeams[i][8]}</p></div>
                            <div className="points"><p>{sortedTeams[i][9]}</p></div>
                        </div>
                    )
                })}
            </div>
